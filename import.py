...
savepath = "%localappdata%/Packages/Packages/Microsoft.BadgerWin10_8wekyb3d8bbwe/SystemAppData/wgs"
...
with open(os.path.join(savepath, "containers.index"), "rb") as index:
  ...
  for j in range(int(count)):
    ...
    with open(os.path.join(newpath, os.listdir(newpath)[len(os.listdir(newpath)[0])!=32]), "wb") as i:
      with open(out, "rb") as k:
        i.write(k.read())